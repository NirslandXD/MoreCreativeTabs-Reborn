...
>>>>>>> 79b2a8a (initial commit)
    ...
    private final HashMap<CreativeModeTab, ResourceLocation> tabBackgrounds = new HashMap<>();
    private final HashMap<String, ResourceLocation> replacementBackgrounds = new HashMap<>();
    private final Set<Item> hiddenItems = new HashSet<>();
    private final Set<String> searchableTabs = new HashSet<>();
    private static final Set<String> PROTECTED_TABS = Set.of("itemgroup.search", "search", "itemgroup.hotbar", "hotbar", "itemgroup.inventory", "inventory");
<<<<<<< HEAD
=======
=======
...
=======
    ...
    private final CreativeModeTab operatorTab = BuiltInRegistries.CREATIVE_MODE_TAB.get(CreativeModeTabsAccessor.getOpBlockTab());
...
<<<<<<< HEAD
                    registerReplacementBackground(location, json);
=======
<<<<<<< HEAD
                    registerReplacementBackground(location, json);
=======
...
>>>>>>> 79b2a8a (initial commit)
                    ...
                    if (json.isSearchBar()) {
                        try {
                            builder.getClass().getMethod("withSearchBar", int.class).invoke(builder, json.getSearchBarWidth());
                    ...
                    registerBackground(location, json, tab);
                    if (json.isSearchBar())
<<<<<<< HEAD
=======
=======
...
<<<<<<< HEAD
            if (item == null || item.getName() == null || item.getName().isBlank()) {
                ModConstants.logger.warn("Skipping item without a name in {}", location);
=======
<<<<<<< HEAD
            if (item == null || item.getName() == null || item.getName().isBlank()) {
                ModConstants.logger.warn("Skipping item without a name in {}", location);
...
>>>>>>> 79b2a8a (initial commit)
    ...
    public boolean processPolicyEntry(ResourceLocation location, Resource resource) {
        try (InputStream stream = resource.open()) {
...
>>>>>>> 79b2a8a (initial commit)
        resourceMap.forEach((location, resource) -> {
            ModConstants.logger.info("Processing {}", location.toString());
            try (InputStream stream = resource.open()) {
                OrderedTabsJsonHelper tabs = GSON.fromJson(new InputStreamReader(stream), OrderedTabsJsonHelper.class);
...
<<<<<<< HEAD
>>>>>>> 79b2a8a (initial commit)
        disabledTabs.removeIf(selector -> customTabs.stream().anyMatch(tab -> tabAliases(tab).contains(selector)));
        disabledTabs.stream().filter(selector -> !knownTabs.contains(selector))
...
=======
=======
        CreativeTabSearchHelper.configure(BuiltInRegistries.CREATIVE_MODE_TAB.get(CreativeModeTabsAccessor.getSearchTab()));
>>>>>>> d16b230 (initial commit)
>>>>>>> 79b2a8a (initial commit)
...
<<<<<<< HEAD
>>>>>>> 79b2a8a (initial commit)
    private void registerBackground(ResourceLocation location, CustomCreativeTabJsonHelper json, CreativeModeTab tab) {
        if (json.getTabBackground() == null || json.getTabBackground().isBlank())
            return;

        tabBackgrounds.put(tab, resolveBackground(location, json));
    }

    private void registerReplacementBackground(ResourceLocation location, CustomCreativeTabJsonHelper json) {
        if (json.getTabBackground() == null || json.getTabBackground().isBlank())
            return;

        ResourceLocation background = resolveBackground(location, json);
        Set<String> aliases = selectorAliases(hasTabReplace(json) ? json.getTabReplace() : legacyFileSelector(location));
        aliases.forEach(alias -> replacementBackgrounds.put(alias, background));
    }

    private ResourceLocation resolveBackground(ResourceLocation location, CustomCreativeTabJsonHelper json) {
        String folder = location.getPath();
        int slash = folder.lastIndexOf('/');
        folder = slash >= 0 ? folder.substring(0, slash + 1) : "";
        return new ResourceLocation(location.getNamespace(), folder + json.getTabBackground());
    }

    public Optional<ResourceLocation> background(CreativeModeTab tab) {
        ResourceLocation background = tabBackgrounds.get(tab);
        if (background != null)
            return Optional.of(background);

        for (String alias : tabAliases(tab)) {
            background = replacementBackgrounds.get(alias);
            if (background != null)
                return Optional.of(background);
        }
...
<<<<<<< HEAD
        return tab.getType() == CreativeModeTab.Type.SEARCH || matchesTab(searchableTabs, tab);
=======
<<<<<<< HEAD
        return tab.getType() == CreativeModeTab.Type.SEARCH || matchesTab(searchableTabs, tab);
=======
    ...
    private boolean isProtectedSearchTab(CreativeModeTab tab) {
        return tab == BuiltInRegistries.CREATIVE_MODE_TAB.get(CreativeModeTabsAccessor.getSearchTab())
                || tab == BuiltInRegistries.CREATIVE_MODE_TAB.get(CreativeModeTabsAccessor.getHotbarTab())
                || tab == BuiltInRegistries.CREATIVE_MODE_TAB.get(CreativeModeTabsAccessor.getInventoryTab());
    }
    ...
    public boolean isManagedTab(CreativeModeTab tab) {
        return customTabs.contains(tab) || replacementTab(tab).isPresent();
>>>>>>> d16b230 (initial commit)
>>>>>>> 79b2a8a (initial commit)
    }
...
=======
    private boolean isProtectedSelector(Set<String> aliases) {
        return aliases.stream().anyMatch(PROTECTED_TABS::contains)
                || matchesTab(aliases, BuiltInRegistries.CREATIVE_MODE_TAB.get(CreativeModeTabsAccessor.getSearchTab()))
                || matchesTab(aliases, BuiltInRegistries.CREATIVE_MODE_TAB.get(CreativeModeTabsAccessor.getHotbarTab()))
                || matchesTab(aliases, BuiltInRegistries.CREATIVE_MODE_TAB.get(CreativeModeTabsAccessor.getInventoryTab()));
    }
...
=======
        ...
        ResourceLocation registryId = BuiltInRegistries.CREATIVE_MODE_TAB.getKey(tab);
        if (registryId != null) {
            aliases.add(registryId.toString().toLowerCase(Locale.ROOT));
            aliases.add(registryId.getPath().toLowerCase(Locale.ROOT));
            aliases.add((registryId.getNamespace() + "." + registryId.getPath()).toLowerCase(Locale.ROOT));
            aliases.add((registryId.getNamespace() + "_" + registryId.getPath()).toLowerCase(Locale.ROOT));
        }
